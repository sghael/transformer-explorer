...
Run `node web/camera-poses.mjs` to record the settled camera pose for every view, representative layer and group/expert sample at 1440×1100 and 390×844, plus each tour chapter, in artifacts/camera-poses/. Set `CAMERA_REPORT` to name the output. Set `CAMERA_BASELINE` to an earlier report to fail on any pose difference above 1e-6. Set `CAMERA_SCREENSHOTS=1` to also capture each view at the Middle layer. Record a baseline before changing camera framing code or data.
