...
Navigation continuity is owned by navigation.ts: scene snapshots retain each node's transform and effective opacity, and the live scene blends between outgoing, contextual and incoming snapshots. Context is retained through early inward travel; newly revealed detail appears before the approach. Camera interpolation uses smooth direction/target changes and logarithmic viewing distance. The selected destination remains independent of the presentation until arrival; reduced motion bypasses the animated snapshots.
