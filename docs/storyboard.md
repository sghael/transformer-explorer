...
| 13–20 s | Stack, then selected layer | Layers repeat the same structure with distinct weights | 32 distinct slices, selected Layer 1–32, spacing then extraction, persistent stack locator | Depth remains legible through extraction; the selected slot stays highlighted; only one assembly opens; repeated geometry does not imply shared weights |
