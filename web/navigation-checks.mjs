...
try {
  ...
  for (const pair of pairs) {
    ...
    await page.evaluate(({ landmark, incoming }) => {
      ...
      const tick = (now) => {
        ...
        if (s) {
          ...
          window.__navigationTrace.push({
            ...
            selected: s.selected,
            camera: s.camera,
            nodes: Object.fromEntries(ids.map((id) => [id, s.nodes[id]])),
          });
        }
        if (
          now - started > 6000 ||
    ...
    const contextFrames = trace.filter(
      (f) => f.phase === "context" && f.presentedView === "layer",
    );
    const losses = [];
    ...
    const assertions = [
      [
        "Context landmarks survive the first300ms of zoom-in",
        () => {
          expect(contextFrames.length).toBeGreaterThan(0);
          expect(early.length).toBeGreaterThan(1);
          for (const frame of early)
            expect(
              visible(frame.nodes[pair.landmark]),
              `${pair.landmark} disappeared at ${frame.elapsedMs.toFixed(1)}ms (${frame.phase})`,
            ).toBe(true);
        },
      ],
      [
